...
We annotated each SV for multiple potential genic effects using an approach described in [the OurDNA SV paper](https://broad.io/gnomad_sv). In brief, this approach considers SV size, class, position, and overlap with exons from canonical transcripts of [GENCODE v19](https://www.gencodegenes.org/human/release_19.html) protein-coding genes.

### Representation in the OurDNA Browser
...
2. ![](https://placehold.it/15/7459B2/000000?text=+) **Intragenic exonic duplication (IED)**: SV is predicted to result in duplicated exons within the gene, without extending beyond the boundaries of the open reading frame. (New in OurDNA v4)
3. **Partial exon duplication (PED)**: SV is predicted to duplicate part of one or more exons without resulting in a whole-gene copy gain. (New in OurDNA v4)
...
4. **Transcription start site duplication**: SV is predicted to result in a duplication of a transcription start site but not result in any other, more severe consequences. (New in OurDNA v4).
...
7. **Exonic breakend**: breakend localizes to exonic locus, but consequence is unclear as SV is incompletely resolved. (New in OurDNA v4).
