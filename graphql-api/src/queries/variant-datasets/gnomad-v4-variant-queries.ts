...
const fetchVariantsAgeDistribution = async (esClient: any, _subset: Subset) => {
  ...
  return {
      exome: (exome_age_distribution && exome_age_distribution.length > 0 ? exome_age_distribution[0] : empty_hist_rec),
      genome: (genome_age_distribution && genome_age_distribution.length > 0 ? genome_age_distribution[0] : empty_hist_rec),
  }
}
