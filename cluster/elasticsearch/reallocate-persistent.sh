...
curl -XPUT $ES_URL/*/_settings -d '{
  "index.routing.allocation.exclude.data_node_hostname": "es-data-temp*",
...
while [[ $(curl -s ${ES_URL}/_cat/shards | grep temp) ]] ; do
  sleep 10
  NUM_SHARDS_TO_ALLOCATE=$(curl -s ${ES_URL}/_cat/shards | grep temp | wc -l)
